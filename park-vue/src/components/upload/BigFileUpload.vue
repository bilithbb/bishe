    ...
    export default {
        ...
        data () {
            return {
                options: {
                    //目标上传 URL，默认POST
                    target: "http://127.0.0.1:9000/common/file/chunk",
                    processParams:(file)=>{
                        return {
                            chunkNumber:file.chunkNumber,
                            currentChunkSize:file.chunkSize,
        ...
        methods: {
            ...
            */
            ...
            onFileSuccess(rootFile, file, response, chunk){
              ...
              this.$message.success(JSON.parse(response).msg);
              this.$emit('input', JSON.parse(response).data.fileName)
            },
        ...
        mounted() {
            const that = this;
            utilMethod.$on('clear',function(data) {
                that.$refs.uploader.files = [];
                that.$refs.uploader.fileList = [];
            })
        }
    }
</script>
